...
def main():
    ...
    assert not cfg.get("hardware", False), (
        "Motion PTQ currently supports software simulation only; "
        "CUDA kernels do not support token-routed A4/A8."
    )
    quant_param_ckpt = torch.load(os.path.join(cfg.save_dir, "./motion_quant_params.pth"), weights_only=True)
    model.load_quant_param_dict(quant_param_ckpt)
    motion_clip_path = os.path.join(cfg.save_dir, "./motion_clip_params.pth")
    if os.path.exists(motion_clip_path):
        motion_clip_ckpt = torch.load(motion_clip_path, weights_only=True)
        model.load_motion_clip_dict(motion_clip_ckpt)
        logger.info(f"loaded motion clip params from {motion_clip_path}")
