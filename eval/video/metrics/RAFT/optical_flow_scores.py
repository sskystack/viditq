...
if __name__ == '__main__':
    ...
    parser.add_argument("--dir_results", type=str, default='', help="")
    parser.add_argument("--metric", type=str, default='celebrity_id_score', help="Specify the metric to be used")
    parser.add_argument('--model', type=str, default='./checkpoints/models/raft-things.pth',help="restore checkpoint")
    ...
    dir_prompts =  '../../prompts/'
    print(f"dir_prompts={dir_prompts}")
    video_paths = [os.path.join(dir_videos, x) for x in os.listdir(dir_videos) if x.endswith('.mp4')]
    ...
    if args.dir_results=='':
        ...
        file_handler = logging.FileHandler(filename=args.dir_results+f"/{metric}_record.txt")
