...
class ImageReward(nn.Module):
    ...
    def score(self, prompt, image):
        ...
        if (os.path.isdir(image)):
            indices, rewards = self.inference_rank(prompt, image)
    ...
    def inference_rank(self, prompt, generations_list, batch_size=64):
        ...
        with torch.no_grad():
            for i, img in tqdm(enumerate(img_dataloader)):
                ...
                image_embeds = self.blip.visual_encoder(img.to(self.device))
                
                # text encode cross attention with image
                image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(self.device)
                text_output = self.blip.text_encoder(text_input.input_ids[ind : ind + img.shape[0]],
                                                        attention_mask = text_input.attention_mask[ind : ind + img.shape[0]],
                                                        encoder_hidden_states = image_embeds,
                                                        encoder_attention_mask = image_atts,
                                                        return_dict = True,
                                                    )
